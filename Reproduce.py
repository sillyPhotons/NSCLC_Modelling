...
from Model import gompertz_ode, predict_volume_doubling_time, predict_KMSC_discrete, predict_KMSC_continuous, discrete_time_tumor_volume_GENG, rk4_tumor_volume
...
# for stage in ["4"]:
    ...
    x = np.around(x)
    px, py = predict_KMSC_discrete(params, np.arange(
        sampling_range[0], sampling_range[1]*31 + Constants.RESOLUTION, Constants.RESOLUTION), pop_manager, rk4_tumor_volume)
